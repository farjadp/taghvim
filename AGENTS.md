...
- **Second clocks.** Tehran is and stays the main clock in the hero. The card beside it may also show the visitor's own time, added automatically only when their IANA zone differs from Tehran by offset, plus up to two cities they pick from `ZONES` in `src/lib/clocks.ts` (persisted as `taghvim-clocks`). A visitor on Tehran time with no cities chosen sees one quiet add link and nothing else. Never move the Tehran clock out of the hero or make a second clock the primary one.
