...
For an isolated worktree preview, use `npm run dev -- --port 3101` and `E2E_PORT=3101 npm run test:e2e`; the override prevents testing the main worktree's server on 3100. Run builds and browser tests sequentially. Do not merge a sandbox branch or remove the original uncommitted sandbox files until Farjad approves the preview.
