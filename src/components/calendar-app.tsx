    ...
    <>
      ...
      <header className="border-b border-line bg-white/80">
        <div className="mx-auto flex min-h-23 max-w-[1240px] flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-8">
          ...
          <button type="button" role="switch" aria-checked={scope === "all"} onClick={toggleScope} className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] transition-colors ${scope === "all" ? "border-forest bg-leaf text-forest" : "border-line text-muted hover:text-forest"}`}><span className={`size-1.5 rounded-full ${scope === "all" ? "bg-forest" : "bg-line"}`} />مناسبت‌های مذهبی و دولتی</button>
        </div>
      </header>
