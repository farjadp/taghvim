...
export function EventsPanel({ year, month, selected, onSelect }: { year: number; month: number; selected: Date; onSelect: (date: Date) => void }) {
  ...
  return (
    <aside className="flex min-w-0 flex-col rounded-[1.75rem] border border-line bg-white">
      ...
      <div className="flex gap-1 px-6 pt-4">{[{ key: "all", label: "همه" }, { key: "iran", label: "ایرانی" }, { key: "religious", label: "مذهبی" }, { key: "world", label: "جهانی" }].map((item) => <button key={item.key} aria-pressed={filter === item.key} onClick={() => setFilter(item.key)} className={`rounded-lg px-3 py-1.5 text-[11px] transition-colors ${filter === item.key ? "bg-leaf font-medium text-forest" : "text-muted hover:bg-paper"}`}>{item.label}</button>)}</div>
