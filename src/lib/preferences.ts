...
// Why: Display preferences — theme, accent colour, font size, font family — as
//      validated values, the localStorage keys that hold them, and the tiny boot
//      script that applies them before first paint so dark mode never flashes.
// Env / Deps: Applied as data-theme / data-accent / data-size / data-font on
...
export type Preferences = { theme: Theme; accent: Accent; size: FontSize; font: Font };
...
export const DEFAULT_PREFERENCES: Preferences = { theme: "auto", accent: "green", size: "md", font: "vazirmatn" };

// One localStorage key per display preference, alongside city, view and clocks.
export const PREFERENCE_KEYS = { theme: "taghvim-theme", accent: "taghvim-accent", size: "taghvim-size", font: "taghvim-font" } as const;
...
export const PREFERENCES_BOOT_SCRIPT = `(function(){var FONT_VALUES=${JSON.stringify(FONTS.map((font) => font.value))},ACCENT_VALUES=${JSON.stringify(ACCENTS.map((accent) => accent.value))};try{var d=document.documentElement,g=function(k){return localStorage.getItem(k)};var t=g("${PREFERENCE_KEYS.theme}"),a=g("${PREFERENCE_KEYS.accent}"),s=g("${PREFERENCE_KEYS.size}"),f=g("${PREFERENCE_KEYS.font}");if(t==="light"||t==="dark")d.setAttribute("data-theme",t);if(a&&a!=="${DEFAULT_PREFERENCES.accent}"&&ACCENT_VALUES.indexOf(a)>-1)d.setAttribute("data-accent",a);if(s==="sm"||s==="lg")d.setAttribute("data-size",s);if(f&&f!=="vazirmatn"&&FONT_VALUES.indexOf(f)>-1)d.setAttribute("data-font",f)}catch(e){}})();`;
